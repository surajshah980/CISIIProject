...
#Instead of binary pneumonia detection, generates output into 3 classes:
#No_Finding, Other Thoracic Pathologies, and Pneumonia
...
#Load the pre-trained model and set number of output features (reduce from 1000 to 3)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in [TRAIN, VAL]:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                #Take the predictions and labels and resize to the binary for
                # further analysis
                new_pred = preds.cpu().numpy()
                new_labels = labels.data.cpu().numpy()
                #Reshape the No_Finding and Other to the same class to see if
                #Accuracy holds for binary detection
                ...
                two_class += np.sum(new_pred == new_labels)
                #print(two_class)
                
            epoch_loss = running_loss / dataset_sizes[phase]
            epoch_acc = running_corrects.double() / dataset_sizes[phase]
            epoch_two_class = two_class / dataset_sizes[phase]

            #Print Accuracies and Losses
            print('{} Loss: {:.4f} Acc: {:.4f} Two Class Acc: {:.4f}'.format(
                phase, epoch_loss, epoch_acc, epoch_two_class))

            # deep copy the model
            if phase == VAL and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model.state_dict(), 'resLarge3Class.pth')
